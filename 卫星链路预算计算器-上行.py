...
class SatelliteLinkCalculator:
    ...
    def generate_report(self):
        ...
        try:
            ...
            base_params = [
                ...
                "terminal_elevation_angle", "terminal_eirp",
                "satellite_antenna_gain", "satellite_noise_figure",
                "satellite_noise_temp",
            ]
    ...
    def get_result_display_name(self, var_name):
        ...
        display_names = {
            ...
            "gt_ratio": "卫星G/T值 (dB/K)",  # 修改为卫星G/T值
        }
...
class InputHandler:
    ...
    def __init__(self, parent):
        ...
        # 定义所有输入参数及其默认值
        self.params = {
            "frequency": tk.StringVar(value="1.71"),  # 频率 (GHz)
            "bandwidth": tk.StringVar(value="0.72"),  # 带宽 (MHz)
            "satellite_height": tk.StringVar(value="400"),  # 卫星高度 (km)
            "terminal_elevation_angle": tk.StringVar(value="90"),  # 终端仰角 (度)
            "terminal_eirp": tk.StringVar(value="23-30-5"),  # 终端EIRP (dBW)
            "satellite_antenna_gain": tk.StringVar(value="30.72"),  # 卫星天线增益 (dBi)
            "satellite_noise_figure": tk.StringVar(value="2.4"),  # 卫星噪声系数 (dB)
            "satellite_noise_temp": tk.StringVar(value="290"),  # 卫星噪声温度 (K)
...
class Calculator:
    ...
    def perform_calculations(self):
        ...
        eirp = ih.get_numeric_value("terminal_eirp")  # 使用终端EIRP而非卫星EIRP
        ...
        ant_gain = ih.get_numeric_value("satellite_antenna_gain")  # 使用卫星天线增益
        nf = ih.get_numeric_value("satellite_noise_figure")  # 使用卫星噪声系数
        t_antenna = ih.get_numeric_value("satellite_noise_temp")  # 使用卫星噪声温度
